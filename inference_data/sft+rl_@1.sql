...
SELECT count(*) , T1.order_status_code , order_id FROM orders AS T1 JOIN order_items AS T2 ON T1.order_id = T2.order_id GROUP BY T1.order_id
...
SELECT customer_first_name , customer_middle_initial , customer_last_name FROM Customers EXCEPT SELECT T1.customer_first_name , T1.customer_middle_initial , T1.customer_last_name FROM Customers AS T1 JOIN Orders AS T2 ORDER BY Customer_id
SELECT T2.order_id , T2.date_order_placed FROM orders_items AS T1 JOIN orders AS T2 ON T1.order_id = T2.order_id GROUP BY T2.order_id HAVING count(*) >= 2
...
SELECT T2.customer_id , T1.customer_first_name , T1.customer_last_name FROM Customers AS T1 JOIN Orders AS T2 ON T1.customer_id = T2.customer_id GROUP BY T2.customer_id HAVING count(DISTINCT T2.order_id) > 2 INTERSECT SELECT T3.customer_id , T4.customer_first_name , T4.customer_last_name FROM Customers AS T4 JOIN Order_items AS T3 ON T4.customer_id = T3.customer_id GROUP BY T3.customer_id HAVING count(*) >= 3
...
SELECT invoice_status_code , invoice_date , T1.shipment_date FROM Invoices AS T1 JOIN shipments AS T2 ON T1.invoice_number = T2.invoice_number
SELECT customer_gender , count(*) FROM customers AS t1 JOIN orders AS t2 JOIN order_items AS t3 JOIN products AS t4 ON t1.customer_id = t2.customer_id AND t2.order_id = t3.order_id AND t3.product_id = t4.product_id GROUP BY customer_gender
...
SELECT t1.product_id , t2.product_name , t2.product_price FROM orders AS t3 JOIN order_items AS t2 ON t3.order_id = t2.order_id JOIN products AS t1 ON t2.product_id = t1.product_id GROUP BY t1.product_id ORDER BY count(*) DESC LIMIT 1
...
SELECT T1.order_id , T1.date_order_placed , T2.total FROM orders AS T1 JOIN (SELECT sum(T3.product_id * T3.product_price) , T1.order_id FROM orders AS T1 JOIN order_items AS T3 ON T1.order_id = T3.order_id JOIN products AS T2 ON T3.product_id = T2.product_id GROUP BY T1.order_id) AS T2 ON T1.order_id = T2.order_id
SELECT count(DISTINCT payment_method_code) FROM Customer_Payment_Methods
SELECT login_name , login_password FROM customers WHERE phone_number LIKE '%+12%'
SELECT login_name , login_password FROM customers WHERE phone_number LIKE '+12%'
SELECT count(DISTINCT order_item_status_code) FROM orders_items
SELECT product_size FROM products WHERE product_name LIKE '%Dell%'
SELECT product_size FROM products WHERE product_name LIKE '%Dell%'
SELECT T3.product_name , T2.shipment_date FROM shipments AS T1 JOIN invoices AS T2 ON T1.invoice_number = T2.invoice_number JOIN orders AS T4 ON T1.order_id = T4.order_id JOIN order_items AS T5 ON T4.order_id = T5.order_id JOIN products AS T3 ON T3.product_id = T5.product_id
SELECT T2.invoice_status_code FROM INVOICES AS T2 JOIN Orders AS T1 ON T2.order_id = T1.order_id JOIN Shipments AS T3 ON T1.order_id = T3.order_id WHERE T2.invoice_status_code != 'Shipped'
SELECT T3.order_id , T1.date_order_placed , sum(T2.order_item_status_code) FROM invoices AS T1 JOIN order_items AS T2 ON T1.invoice_number = T2.order_id JOIN orders AS T3 ON T2.order_id = T3.order_id GROUP BY T3.order_id
...
SELECT DISTINCT T1.product_name , T1.product_price , T1.product_description FROM products AS T1 JOIN orders AS T2 JOIN customers AS T3 ON T2.customer_id = T3.customer_id AND T2.product_id = T1.product_id WHERE T3.gender_code = 'F'
...
SELECT T3.product_name , T1.customer_first_name FROM customers AS T1 JOIN orders AS T2 JOIN order_items AS T3 ON T1.customer_id = T2.customer_id AND T2.order_id = T3.order_id
...
SELECT avg(T2.product_price) FROM orders AS T1 JOIN order_items AS T2 ON T1.order_id = T2.order_id JOIN products AS T3 ON T2.product_id = T3.product_id
...
SELECT T2.invoice_date , T1.invoice_number FROM invoices AS T1 JOIN shipments AS T2 ON T1.invoice_number = T2.invoice_number GROUP BY T2.invoice_number HAVING count(*) >= 2
...
SELECT t1.name FROM channel AS t1 JOIN director AS t2 ON t1.director_id = t2.director_id WHERE t2.age < 40 INTERSECT SELECT t1.name FROM channel AS t1 JOIN director AS t2 ON t1.director_id = t2.director_id WHERE t2.age > 60
...
SELECT Model FROM vehicle ORDER BY Top_Speed DESC LIMIT 1 WHERE Power > 6000
SELECT Top_Speed , Power FROM vehicle WHERE Build_Year = 1996
SELECT Top_Speed , Power FROM vehicle WHERE Build_Year = 1996
SELECT T1.Driver_ID , T2.Total_Production FROM driver AS T1 JOIN vehicle_driver AS T2 ON T1.Driver_ID = T2.Driver_ID GROUP BY T1.Driver_ID ORDER BY Total_Production DESC LIMIT 1
...
SELECT T1.Exam_Name , T1.Exam_Date FROM EXAMS AS T1 JOIN SUBJECTS AS T2 ON T1.Subject_Code = T2.Area_Code WHERE T2.Area_Code != Database
SELECT DISTINCT Student_Answer_Text FROM Student_Answers WHERE COMMENTS = Normal
SELECT Student_Answer_Text FROM Student_Answers GROUP BY Student_Answer_Text ORDER BY COUNT(*) DESC
...
SELECT LastName FROM students WHERE Gender_MFU = F OR Gender_MFU = M
SELECT T3.Vehicle_ID , T3.Model FROM vehicle_driver AS T1 JOIN driver AS T2 ON T1.Driver_ID = T2.Driver_ID JOIN vehicle AS T3 ON T1.Vehicle_ID = T3.Vehicle_ID WHERE T2.Name = Jeff Gordon OR count(*) > 2 GROUP BY T1.Vehicle_ID
...
SELECT 1 FROM EXAMS GROUP BY Date_of_Answer ORDER BY COUNT (*) DESC LIMIT 1
...
SELECT DISTINCT t1.customer_name FROM customers AS t1 JOIN customer_orders AS t2 ON t1.customer_id = t2.customer_id GROUP BY t2.payment_method_code ORDER BY count(*) , t1.customer_name DESC LIMIT 2
SELECT T1.order_id , T1.order_date , T1.order_status_code FROM customer_orders AS T1 JOIN customers AS T2 ON T1.customer_id = T2.customer_id WHERE T2.customer_name = Jeromy
SELECT order_status_code , count(*) FROM customer_orders GROUP BY order_status_code
SELECT T2.customer_name , T1.customer_id , count(*) FROM customer_orders AS T1 JOIN customers AS T2 ON T1.customer_id = T2.customer_id GROUP BY T1.customer_id
...
SELECT T1.Service_details FROM Services AS T1 JOIN Customers_Services AS T2 ON T1.Service_ID = T2.Service_ID JOIN Customers AS T3 ON T2.Customer_ID = T3.Customer_ID WHERE T3.Customer_Details = Hardy Kutch
...
SELECT T1.customer_details FROM Customers AS T1 JOIN Customer_Interaction AS T2 ON T1.Customer_ID = T2.Customer_ID WHERE T2.Status_Code = Stuck AND Services_and_Channels_Details = bad
...
SELECT name , TYPE_of_powertrain , Annual_fuel_cost FROM VEHICLES WHERE Model_year = 2013 OR Model_year = 2014
SELECT DISTINCT T3.Service_Details FROM Customer_Interaction AS T1 JOIN CHANNEL AS T2 ON T1.Channel_ID = T2.Channel_ID JOIN Services AS T3 ON T1.Service_ID = T3.Service_ID WHERE T2.Channel_Details = 15 ij
...
SELECT name , age , membership_credit FROM Customers
SELECT T1.Service_id FROM Services AS T1 JOIN Customers AS T2 ON T1.Customer_id = T2.Customer_id WHERE T2.Customer_details = Hardy Kutch OR EXISTS (SELECT T4.Service_id FROM Customer_Interactions AS T3 JOIN Services AS T4 ON T3.Service_id = T4.Service_id JOIN Customers AS T5 ON T3.Customer_ID = T5.Customer_ID WHERE T3.status_code = good AND T5.Customer_details = Hardy Kutch )
SELECT min(annual_fuel_cost) , max(annual_fuel_cost) , avg(annual_fuel_cost) FROM vehicles
...
SELECT avg(T1.age) FROM customers AS T1 JOIN renting_history AS T2 ON T1.id = T2.customer_id WHERE T1.memberhip_credit > (SELECT avg(memberhip_credit) FROM customers)
...
SELECT T2.student_details , Ref_Address_Types.address_type_description FROM Students_Addresses AS T1 JOIN Addresses AS T2 ON T1.address_id = T2.address_id JOIN Ref_Address_Types ON T1.address_type_code = Ref_Address_Types.address_type_code
...
SELECT T2.achievement_details , T1.transcript_id FROM TRANSCRIPTS AS T1 JOIN STUDENTS AS T2 ON T1.student_id = T2.student_id GROUP BY T2.transcript_id ORDER BY COUNT(*) DESC LIMIT 1
...
SELECT count(*) , T1.event_type_code , T2.event_type_description FROM Student_Events AS T1 JOIN Ref_event_Types AS T2 ON T1.event_type_code = T2.event_type_CODE GROUP BY T1.event_type_code ORDER BY count(*) DESC LIMIT 1
SELECT T1.bio_data FROM Students AS T1 JOIN Behaviour_monitoring AS T2 ON T1.student_id = T2.student_id GROUP BY T1.student_id HAVING count(*) = (SELECT count(*) FROM behaviour_monitoring GROUP BY T2.student_id ORDER BY count(*) DESC LIMIT 1) ORDER BY T1.student_id
SELECT date_of_transcript , transcript_details FROM TRANSCRIPTS
SELECT T3.student_details , T1.address_type_description FROM Students_Addresses AS T1 JOIN Addresses AS T2 ON T1.address_id = T2.address_id JOIN Students AS T3 ON T1.student_id = T3.student_id
SELECT date_of_transcript , transcript_details FROM TRANSCRIPTS
SELECT T1.achievement_details , T2.achievement_type_description FROMAchievements AS T1 JOIN Ref_Achievement_Type AS T2 ON T1.achievement_type_code = T2.achievement_type_code
...
SELECT other_details , date_achievement , achievement_type_code FROMAchievements
SELECT student_id , count(*) FROM Classes GROUP BY student_id ORDER BY count(*) DESC LIMIT 1
SELECT achievement_type_code , achievement_details , date_achievement FROMAchievements
SELECT DISTINCT T3.teacher_details , T2.student_details FROM Students AS T1 JOIN Classes AS T2 ON T1.student_id = T2.student_id JOIN Teachers AS T3 ON T3.teacher_id = T2.teacher_id
...
SELECT T2.class_details , T1.class_details FROM Teachers AS T1 JOIN Classes AS T2 ON T1.teacher_id = T2.teacher_id JOIN Students AS T3 ON T2.student_id = T3.student_id
...
SELECT T1.student_id , T1.student_details FROM Students AS T1 JOIN CLASS AS T2 ON T1.student_id = T2.student_id GROUP BY T1.student_id HAVING count(*) = 2
...
SELECT class_id , student_id FROM TRANSCRIPTS GROUP BY student_id ORDER BY count(*) DESC LIMIT 1
SELECT date_of_loan FROM Student_loans ORDER BY date_of_loan ASC LIMIT 1
...
SELECT T2.teacher_id , MAX(T1.class_count) FROM (SELECT count(*) , T2.teacher_id FROM Classes AS T1 JOIN Teachers AS T2 ON T1.teacher_id = T2.teacher_id GROUP BY T2.teacher_id) AS T1 JOIN Teachers AS T2 ON T1.teacher_id = T2.teacher_id GROUP BY T2.teacher_id
...
SELECT T2.teacher_details FROM TRANSCRIPTS AS T1 JOIN TEACHERS AS T2 ON T1.teacher_id = T2.teacher_id ORDER BY T1.date_of_transcript LIMIT 1
...
SELECT T1.student_id , T1.bio_data , count(*) FROM Students AS T1 JOIN Courses AS T2 ON T1.student_id = T2.student_id GROUP BY T1.student_id
SELECT T1.address_type_code , T2.address_type_description FROM Ref_Address_Types AS T1 JOIN Students_Addresses AS T3 ON T1.address_type_code = T3.address_type_code JOIN Addresses AS T2 ON T3.address_id = T2.address_id GROUP BY T3.address_type_code ORDER BY count(*) DESC LIMIT 1
SELECT datetime_detention_start FROM Detention
SELECT name FROM author
SELECT datetime_detention_start FROM Detention
SELECT DISTINCT T3.teacher_details FROM TRANSCRIPTS AS T1 JOIN STUDENTS AS T2 JOIN CLASSES AS T3 ON T1.student_id = T2.student_id AND T2.student_id = T3.student_id WHERE T1.date_of_transcript = ( SELECT min(date_of_transcript) FROM TRANSCRIPTS )
SELECT name FROM author
SELECT name , address FROM Client
SELECT name , address FROM Client
SELECT DISTINCT title , ISBN , SalePrice FROM BOOK
SELECT T1.student_id , T1.bio_data FROM Students AS T1 JOIN Student_Events AS T2 ON T1.student_id = T2.student_id EXCEPT SELECT student_id FROM Student_Loans
SELECT count(*) FROM Book
SELECT count(*) FROM BOOK
SELECT Title , ISBN , SalePrice FROM BOOK
SELECT count(*) FROM Client
SELECT T1.date_from , T2.date_to FROM Students_Addresses AS T1 JOIN Students_Addresses AS T2 ON T1.student_id = T2.student_id JOIN Transcripts AS T3 ON T1.student_id = T3.student_id GROUP BY T1.student_id HAVING count(date_from) >= 2
SELECT count(*) FROM author
SELECT count(*) FROM author
SELECT count(*) FROM Client
SELECT T3.datetime_detention_start , T3.datetime_detention_end FROM Detention AS T1 JOIN Students AS T2 ON T1.student_id = T2.student_id JOIN Student_Events AS T3 ON T1.student_id = T3.student_id GROUP BY T3.student_id HAVING count(*) = 2
...
SELECT T2.title , T1.name FROM author AS T1 JOIN author_book AS T2 ON T1.idauthor = T2.author JOIN book AS T3 ON T2.isbn = T3.isbn
SELECT DISTINCT T1.isbn , sum(T2.amount) FROM book AS T1 JOIN Books_Order AS T2 ON T1.isbn = T2.isbn GROUP BY T1.isbn
SELECT T3.title , T2.name FROM author AS T1 JOIN author_book AS T2 ON T1.idauthor = T2.author JOIN book AS T3 ON T2.isbn = T3.isbn
SELECT T2.ID ORDER , count(*) FROM orders AS T1 JOIN Books_order AS T2 ON T1.idorder = T2.idorder GROUP BY T1.idorder
SELECT ISBLP , count(*) FROM Books_order AS T1 JOIN book AS T2 ON T1.ISBN = T2.ISBN GROUP BY T1.ISBN
...
SELECT max(SalePrice) , min(SalePrice) FROM Book
SELECT avg(purchaseprice) , avg(saleprice) FROM book
SELECT T1.title FROM book AS T1 JOIN book ORDER AS T2 ON T1.isbn = T2.isbn GROUP BY T1.title ORDER BY sum(T2.amount) DESC LIMIT 1
SELECT T1.name , sum(T4.amount) FROM client AS T1 JOIN orders AS T2 ON T1.idClient = T2.idClient JOIN Books_Order AS T3 ON T3.idOrder = T2.idOrder JOIN book AS T4 ON T4.isbn = T3.isbn GROUP BY T1.name
...
SELECT title FROM book WHERE saleprice > (SELECT avg(saleprice) FROM book)
SELECT name FROM CLIENT EXCEPT SELECT T1.name FROM CLIENT AS T1 JOIN ORDER AS T2 ON T1.idclient = T2.idclient
...
SELECT T3.SalePrice FROM Author AS T1 JOIN Author_Book AS T2 ON T1.idAuthorA = T2.Author JOIN Book AS T3 ON T2.Isbn = T3.ISBN WHERE T1.Name = Plato
SELECT t2.name FROM BOOK AS t1 JOIN author AS t2 ON t1.author = t2.idauthor WHERE t1.title = Pride and Prejudice
SELECT T1.SalePrice FROM book AS T1 JOIN AUTHOR_BOOK AS T2 ON T1.ISBN = T2.ISBN JOIN author AS T3 ON T2.Author = T3.idAuthorA WHERE T3.name = Plato
...
SELECT t2.name FROM book AS t1 JOIN Author_Book AS t2 ON t1.isbn = t2.isbn JOIN author AS t3 ON t2.author = t3.idauthor WHERE t1.title = Pride and Prejudice
SELECT T3.Title FROM Author_Book AS T1 JOIN BOOK AS T2 ON T1.ISBN = T2.ISBN JOIN AUTHOR AS T1 ON T1.Author = T1.idAuthor WHERE T1.Name LIKE %Plato%
...
SELECT t3.title FROM author AS t1 JOIN author_book AS t2 ON t1.idauthor = t2.author JOIN book AS t3 ON t3.isbn = t2.isbn WHERE t1.name = Plato AND t2.isbn IN (SELECT T1.A FROM book AS T1 JOIN author_Book AS T2 ON T2.isbn = T1.isbn WHERE T1.author = Plato GROUP BY T2.isbn HAVING avg(T1.SalePrice) < (SELECT avg(SalePrice) FROM book))
...
SELECT T5.isbn FROM orders AS T1 JOIN book AS T2 JOIN books_order AS T5 ON T1.idorder = T5.idorder JOIN CLIENT AS T3 ON T1.idclient = T3.idclient JOIN order AS T4 ON T4.idorder = T5.idorder JOIN CLIENT AS T6 ON T4.idclient = T6.idclient WHERE T3.name = Peter Doe INTERSECT SELECT T5.isbn FROM orders AS T1 JOIN book AS T2 JOIN books_order AS T5 ON T1.idorder = T5.idorder JOIN CLIENT AS T3 ON T1.idclient = T3.idclient JOIN order AS T4 ON T4.idorder = T5.idorder JOIN CLIENT AS T6 ON T4.idclient = T6.idclient WHERE T6.name = James Smith
SELECT DISTINCT T1.Name FROM Client AS T1 JOIN orders AS T2 ON T1.idclient = T2.idclient JOIN Books ORDER AS T3 ON T2.idorder = T3.idorder WHERE T3.title = Pride and Prejudice
...
SELECT rank FROM book ORDER BY pages LIMIT 1
SELECT count(DISTINCT TYPE) FROM book
SELECT T1Title FROM Books AS T1 JOIN Books_Order AS T2 JOIN Orders AS T3 ON T1.isbn = T2.isbn AND T3.idorder = T2.idorder JOIN Client AS T4 ON T3.idclient = T4.idclient WHERE T4.Name = Peter Doe EXCEPT SELECT T1Title FROM Books AS T1 JOIN Books_ORDER AS T2 JOIN Orders AS T3 ON T1.isbn = T2.isbn AND T3.idorder = T2.idorder JOIN CLIENT AS T4 ON T3.idclient = T4.idclient WHERE T4.Name = James Smith
...
SELECT t3.team_leader FROM club AS t1 JOIN club_leader AS t2 ON t1.club_id = t2.club_id JOIN member AS t3 ON t2.member_id = t3.member_id WHERE t1.overall_ranking > 100
...
SELECT T3.Team_Leader , T2.Club_Name FROM club_leader AS T1 JOIN club AS T2 ON T1.Club_ID = T2.Club_ID JOIN member AS T3 ON T1.Member_ID = T3.Member_ID
SELECT document_subset_name FROM Document_subsets
...
SELECT T1.Collection_Subset_Name FROM Collection_Subsets AS T1 JOIN Collections AS T2 ON T1.Collection_id = T2.Collection_ID
...
SELECT DISTINCT parent_document_description FROM document_objects
...
SELECT t1.collecion_description FROM collection_subsets AS t3 JOIN collections AS t1 JOIN collection_subset_members AS t2 ON t3.collection_subset_id = t2.collection_subset_id AND t3.collection_id = t2.collection_id AND t1.collection_id = t2.collection_id WHERE t2.collection_id = t1.collection_id AND t3.collection_subset_name = Top Collection
SELECT count(*) , parent_document_object_id FROM Document_Objects GROUP BY parent_document_object_id
SELECT Collection_Name FROM Collections
SELECT collection_description FROM collections WHERE collection_name = Best
SELECT collection_description FROM collections WHERE collection_name = Best
SELECT parent_document_id , count(*) FROM Document_Objects GROUP BY parent_document_id
...
SELECT distinct T1.owner FROM Document_Objects AS T1 JOIN Document_Objects AS T2 ON T1.document_object_id = T2.related_document_object_id WHERE T2.owner = Ransom
SELECT T1.collection_name FROM collections AS T1 JOIN documents_in_collections AS T2 ON T1.collection_id = T2.collection_id WHERE T2.document_object_id IN (SELECT T1.document_object_id FROM document_subsets AS T1 JOIN document_subset_members AS T2 ON T1.document_subset_id = T2.document_subset_id JOIN documents_in_collections AS T3 ON T3_document_object_id = T2.related_document_object_id GROUP BY T3.document_object_id HAVING count(*) > 1) AND T1.collection_name = Nice
SELECT document_object_id , count(*) FROM documents_in_collections WHERE count(*) BETWEEN 2 AND 4 GROUP BY document_object_id
...
SELECT T2.Collection_Subset_ID , T2.Collection_Subset_Name , count(*) FROM Collections AS T1 JOIN Collection_Subsets AS T2 ON T1.Collection_ID = T2.Collection_ID GROUP BY T2.Collection_Subset_ID
SELECT T1.collection_subsets_id , T1.collection_subset_name , count(*) FROM collection_subsets AS T1 JOIN collection_subset_members AS T2 ON T1.collection_subsets_id = T2.collection_subsets_id GROUP BY T1.collection_subsets_id
...
SELECT t2.document_subset_name FROM document_objects AS t1 JOIN document_subset_members AS t2 ON t1.document_object_id = t2.document_object_id JOIN document_subsets AS t3 ON t2.document_subset_id = t3.document_subset_id WHERE t1.owner = Braeden GROUP BY t2.document_subset_name
...
SELECT document_subset_id , document_subset_name , count(*) FROM Document_Subsets AS T1 JOIN Document_Subset_Members AS T2 ON T1.document_subset_id = T2.document_subset_id GROUP BY document_subset_id
...
SELECT count(*) FROM Document_Objects AS T1 JOIN Documents_in_collections AS T2 ON T1.document_Object_id = T2.document_object_id JOIN Collections AS T3 ON T3.collection_id = T2.collection_id WHERE T3.collection_name = 'Best'
SELECT document_id FROM Document_objects AS T1 JOIN document_subset_members AS T2 ON T1.document object_id = T2.document object_id JOIN document_subsets AS T3 ON T2.document subset_id = T3.document subset_id WHERE T3.document_subset_name = 'Best for 2000'
...
SELECT document_object_id FROM Document_subset_members AS T1 JOIN document_objects AS T2 ON T1.document_object_id = T2.document_object_id JOIN document_subsets AS T3 ON T1.document_subset_id = T3.document_subset_id WHERE T3.document_subset_name = Best for 2000
...
SELECT T1.document_subsetName , T3.document_object_id FROM document_in_collections AS T1 JOIN document_objects AS T2 ON T1.document_object_id = T2.document_object_id JOIN document_objects AS T3 ON T3.document_object_id = T2.related_document_object_id JOIN document_subsets AS T4 ON T4.document_subset_id = T3.document_subset_id
...
SELECT DISTINCT document_object_id FROM Document_Objects AS T1 JOIN Document_Subset_Members AS T2 ON T1.document_object_id = T2.document_Object_ID JOIN Document_Subsets AS T3 ON T2.Document_Subset_ID = T3.Document_Subset_ID WHERE T3.Document_Subset_Name = 'Best for 2000' INTERSECT SELECT DISTINCT T1.document_object_id FROM Document_Objects AS T1 JOIN Documents_in_Collections AS T2 ON T1.document_object_id = T2.document_object_id JOIN Collections AS T3 ON T2.Collection_ID = T3.Collection_ID WHERE T3.Collection_Name = 'Best'
SELECT count(*) FROM songs WHERE name LIKE %Love%
SELECT name , language FROM songs
SELECT count(*) FROM collections AS T1 JOIN collection_subset_members AS T2 ON T1.collection_id = T2.collection_id JOIN collection_subsets AS T3 ON T2.collection_subset_id = T3.collection_subset_id WHERE T1_COLLECTION_name = Best
SELECT t2.collection_name FROM collection_subset_members AS t1 JOIN collections AS t2 ON t1.collection_id = t2.collection_id JOIN collection_subsets AS t3 ON t1.collection_subset_id = t3.collection_subset_id WHERE t3.collecrtion_subset_name = Best
SELECT max(voice_sound_quality) , min(voice_sound_quality) FROM performance_score
SELECT DISTINCT t1.collection_subset_name FROM collection_subsets AS t1 JOIN collection_subset_members AS t2 ON t1.collection_id = t2.collection_subset_id JOIN collections AS t3 ON t3.collection_id = t2.collection_id WHERE t3.collection_name = Best
...
SELECT T3.voice_sound_quality , T3.rhythm_tempo , T3.stage_presence FROM performance_score AS T1 JOIN songs AS T2 ON T1.songs_id = T2.id JOIN participants AS T3 ON T1.participant_id = T3.id WHERE T3.name = 'Freeway'
SELECT DISTINCT T3.stage_presence FROM songs AS T1 JOIN performance_score AS T2 ON T1.id = T2.songs_id JOIN participants AS T3 ON T2.participant_id = T3.id WHERE T1.language = English
SELECT T1.collection_name , T3.collection_name FROM collections AS T1 JOIN collectionsubset_members AS T2 ON T1.collection_id = T2.collection_id JOIN collection_subsets AS T3 ON T2.collection_subset_id = T3.collection_subset_id WHERE T1.collection_name = Best
...
SELECT document_object_id FROM Document_Objects WHERE document_object_id IN ( SELECT T1.document_object_id FROM Document_Subset_Members AS T1 JOIN Document_Subsets AS T2 ON T1.document_subsets_id = T2.document_subsets_id WHERE T2.document_subset_name = Best for 2000 ) UNION SELECT document_object_id FROM Documents_in_Collections WHERE T2.collection_name = Best
...
SELECT latitude , longitude FROM city WHERE city_name = Baltimore
SELECT T3.original_artist FROM performance_score AS T1 JOIN songs AS T2 ON T1.songs_id = T2.id JOIN participants AS T3 ON T1.participant_id = T3.id WHERE T1.rhythm_tempo > 5 ORDER BY T1.voice_sound_quality
SELECT city_name FROM city WHERE state = PA
SELECT city_name FROM city WHERE state = 'PA'
SELECT city_name FROM city WHERE country = USA ORDER BY latitude
SELECT count(*) FROM city WHERE country = Canada
SELECT T3.voice_sound_quality FROM songs AS T1 JOIN performance_score AS T2 ON T1.id = T2.song_id JOIN participants AS T3 ON T3.id = T2.participant_id WHERE T1.name = 'The Balkan Girls' AND T1.language = 'english'
SELECT city_name FROM city WHERE country = USA ORDER BY latitude
...
SELECT T3.city_name , avg(T2.distance) FROM direct_distance AS T1 JOIN city AS T3 ON T1.city1_code = T3.city_code JOIN city AS T2 ON T1.city2_code = T2.city_code GROUP BY T1.city1_code
...
SELECT DISTINCT sid FROM Reserves GROUP BY sid HAVING count(DISTINCT name) >= 1
SELECT city2_code FROM city WHERE city_name = Linda AND city_code IN (SELECT city1_code FROM direct_distance WHERE city2_code IN (SELECT city_code FROM city WHERE city_name = Tracy AND city_code IN (SELECT city1_code FROM direct_distance WHERE city2_code IN (SELECT city_code FROM city WHERE city_name = Smith ))))
...
SELECT T1.sid FROM reserves AS T1 JOIN boats AS T2 ON T1.bid = T2.bid WHERE T2.color = red OR T2.color = blue
SELECT DISTINCT sid FROM Reserves EXCEPT SELECT DISTINCT sid FROM Reserves WHERE sid IN (SELECT DISTINCT sid FROM Reserves WHERE age = (SELECT min(age) FROM Sailors))
...
SELECT T2.name , T1.sailorsid FROM Reserves AS T1 JOIN Boats AS T2 ON T1.bid = T2.bid JOIN Sailors AS T3 ON T1.sid = T3.sid WHERE T2.color = red OR T2.color = blue
...
SELECT avg(rating) , max(age) FROM Sailors
SELECT T3.name , T2.sid FROM Reserves AS T1 JOIN Boats AS T2 ON T1.bid = T2.bid JOIN Sailors AS T3 ON T1.sid = T3.sid WHERE T2.name = red INTERSECT SELECT T3.name , T2.sid FROM Reserves AS T1 JOIN Boats AS T2 ON T1.bid = T2.bid JOIN Sailors AS T3 ON T1.sid = T3.sid WHERE T2.name = blue
SELECT count(*) , T1.name FROM boats AS T1 JOIN reserves AS T2 ON T1.bid = T2.bid GROUP BY T2.bid
SELECT name , sid FROM RESERVES AS T1 JOIN Sailors AS T2 ON T1.sid = T2.sid WHERE T2.rating >= 3
...
SELECT count(*) , T1.name FROM Reserves AS T1 JOIN Boats AS T2 ON T1.bid = T2.bid GROUP BY T1.bid HAVING count(*) > 1
...
SELECT T2.rating , avg(T1.age) FROM Reserves AS T1 JOIN Sailors AS T2 ON T1.sid = T2.sid JOIN Boats AS T3 ON T3.bid = T1.bid WHERE T3.color = red GROUP BY T2.rating
SELECT name , age FROM Sailors ORDER BY rating DESC
...
SELECT CLASS FROM headphone GROUP BY CLASS ORDER BY count(*) DESC LIMIT 1
SELECT avg(T1.age) , T1.rating FROM Reserves AS T1 JOIN Boats AS T2 ON T1.bid = T2.bid WHERE T2.color = 'red' GROUP BY T1.rating
...
SELECT model , CLASS , construction FROM headphone ORDER BY price ASC LIMIT 1
SELECT DISTINCT name FROM Reserves AS T1 JOIN Boats AS T2 ON T1.bid = T2.bid GROUP BY T1.sid HAVING avg(T2.color) > (SELECT max(rating) FROM Reserves AS T3 JOIN Boats AS T4 ON T3.bid = T4.bid WHERE T4.color = red )
...
SELECT T1.Earpads FROM stock AS T1 JOIN headphone AS T2 ON T1.Headphone_ID = T2.Headphone_ID GROUP BY T1.Earpads ORDER BY count(*) DESC LIMIT 2
...
SELECTName FROM store WHERE Store_ID NOT IN (SELECT Store_ID FROM stock)
...
SELECT name , address FROM affiliation
SELECT DISTINCT Neighbourhood FROM store EXCEPT SELECT DISTINCT T1.Neighbourhood FROM store AS T1 JOIN stock AS T2 ON T1.Store_ID = T2.Store_ID
...
SELECT T2.name FROM paper AS T1 JOIN author AS T3 ON T1.paper_id = T3.paper_id JOIN affiliation AS T2 ON T2.affiliation_id = T3.affiliation_id GROUP BY T2.name ORDER BY count(*) ASC
SELECT DISTINCT COUNT ( t3.paper_id ) , t1.name FROM paper AS t3 JOIN author AS t2 ON t3.paper_id = t2.paper_id JOIN author AS t1 ON t3.paper_id = t1.paper_id GROUP BY t1.name HAVING COUNT ( DISTINCT t3.paper_id ) > 50
...
SELECT DISTINCT COUNT ( DISTINCT t3.paper_id ) , t2.name FROM author AS t1 JOIN author_list AS t2 ON t1.author_id = t2.author_id JOIN paper AS t3 ON t2.paper_id = t3.paper_id GROUP BY t2.name HAVING COUNT ( DISTINCT t3.paper_id ) = 1
...
SELECT count(DISTINCT t3.cited_paper_id) FROM paper AS t1 JOIN citation AS t2 ON t1.paper_id = t2.paper_id JOIN paper AS t3 ON t2.cited_paper_id = t3.paper_id WHERE t1.paper_id = D12-1027
...
SELECT t3.paper_id , COUNT ( t4.cited_paper_id ) FROM paper AS t1 JOIN citation AS t4 ON t1.paper_id = t4.cited_paper_id JOIN author AS t2 ON t2.author_id = t3.paper_id JOIN author_list AS t3 ON t3.paper_id = t2.paper_id JOIN affiliation AS t5 ON t3.affiliation_id = t5.affiliation_id GROUP BY t3.paper_id ORDER BY COUNT ( t4.cited_paper_id ) DESC LIMIT 1
SELECT DISTINCT COUNT ( t4.cited_paper_id ) FROM paper AS t3 JOIN author AS t2 ON t3.affiliation_id = t2.affiliation_id JOIN author_list AS t1 ON t1.paper_id = t3.paper_id JOIN citation AS t4 ON t3.paper_id = t4.paper_id WHERE t2.name = Mckeown , Kathleen
SELECT t3.cited_paper_id , COUNT ( t3.cited_paper_id ) FROM paper AS t1 JOIN citation AS t2 ON t1.paper_id = t2.cited_paper_id JOIN paper AS t3 ON t2.paper_id = t3.paper_id GROUP BY t3.cited_paper_id ORDER BY COUNT ( t3.cited_paper_id ) DESC LIMIT 10
...
SELECT t4.name , count (*) FROM author AS t1 JOIN author_list AS t2 ON t1.author_id = t2.author_id JOIN paper AS t3 ON t2.paper_id = t3.paper_id JOIN citation AS t5 ON t3.paper_id = t5.cited_paper_id JOIN paper AS t6 ON t5.paper_id = t6.paper_id GROUP BY t1.author_id ORDER BY count (*) DESC LIMIT 1
...
SELECT count(*) FROM paper AS t1 JOIN citation AS t2 ON t1.paper_id = t2.cited_paper_id JOIN author AS t4 ON t4.paper_id = t1.paper_id JOIN author_list AS t3 ON t4.paper_id = t3.paper_id WHERE t3.author_id = t4.author_id AND t3.paper_id = t4.paper_id AND t4.name = Mckeown , Kathleen
...
SELECT t3.title FROM author_list AS t2 JOIN paper AS t3 ON t2.paper_id = t3.paper_id JOIN author AS t1 ON t2.auth_id = t1.auth_id GROUP BY t3.paper_id ORDER BY count(*) DESC LIMIT 1
SELECT paper_id FROM paper WHERE title LIKE %translation%
...
SELECT DISTINCT COUNT ( DISTINCT t1.author_id ) FROM affiliation AS t4 JOIN author AS t2 ON t4.affiliation_id = t2.affiliation_id JOIN author AS t3 ON t3.author_id = t1.author_id JOIN author_list AS t5 ON t5.author_id = t3.author_id JOIN paper AS t6 ON t6.paper_id = t5.paper_id JOIN author AS t1 ON t1.author_id = t5.paper_id WHERE t2.name = Mckeown , Kathleen
...
SELECT DISTINCT COUNT ( t3.paper_id ) , t2.name FROM affiliation AS t2 JOIN author AS t1 ON t2.affiliation_id = t1.affiliation_id JOIN paper AS t3 ON t3.paper_id = t1.paper_id GROUP BY t2.name
...
SELECT venue , COUNT(*) , DISTINCT YEAR FROM paper GROUP BY venue , YEAR
SELECT count(*) FROM conference
SELECT count(*) FROM paper WHERE citation_count <= 50
SELECT count(*) FROM conference
...
SELECT name FROM author WHERE author_id NOT IN ( SELECT DISTINCT t1.author_id FROM paper AS t3 JOIN author_list AS t1 ON t3.paper_id = t1.paper_id JOIN venue AS t2 ON t2.venue = t3.venue WHERE t2.venue = ACL )
SELECT Conference_Name , YEAR , LOCATION FROM conference
...
SELECT DISTINCT t1.name FROM author AS t1 JOIN paper AS t2 ON t1.author_id = t2.author_id WHERE t2.year = 2009 INTERSECT SELECT DISTINCT t1.name FROM author AS t1 JOIN paper AS t2 ON t1.author_id = t2.author_id WHERE t2.year = 2009 AND t2.venue = ACL
...
SELECT t4.name FROM paper AS t2 JOIN citation AS t5 ON t2.paper_id = t5.cited_paper_id JOIN paper AS t3 ON t3.paper_id = t5.paper_id JOIN author AS t1 ON t1.author_id = t3.author_id JOIN author_list AS t6 ON t6.paper_id = t3.paper_id AND t6.author_id = t1.author_id JOIN affiliation AS t7 ON t7.affiliation_id = t6.affiliation_id JOIN paper AS t8 ON t8.paper_id = t5.cited_paper_id WHERE t2.venue = NAACL AND t8.venue = ACL AND t2.year = 2009 GROUP BY t4.name HAVING COUNT ( DISTINCT t2.paper_id ) > 1
SELECT count(*) FROM institution WHERE founded BETWEEN 1850 AND 1900
SELECT count(*) FROM institution WHERE founded BETWEEN 1850 AND 1900
SELECT institution_name , LOCATION FROM institution ORDER BY founded DESC LIMIT 1
SELECT institution_name , LOCATION FROM institution ORDER BY founded DESC LIMIT 1
SELECT COUNT(*) FROM paper AS t1 JOIN citation AS t2 ON t1.paper_id = t2.cited_paper_id JOIN author AS t3 ON t3.author_id = t1.author_id GROUP BY t1.title HAVING COUNT(*) <= 50
SELECT institution_name , LOCATION , founded FROM institution
SELECT LOCATION FROM conference GROUP BY LOCATION HAVING count(*) >= 2
...
SELECT T2.Institution_Name , COUNT(*) FROM staff AS T1 JOIN institution AS T2 ON T1.institution_id = T2.Institution_ID WHERE T2Founded > 1800 GROUP BY T1.institution_id
...
SELECT pilot_name FROM PilotSkills ORDER BY age ASC WHERE age < ( SELECT avg(age) FROM pilotSkills )
SELECT count(DISTINCT location) FROM hangar
...
SELECT DISTINCT T2.pilot_name FROM pilotskills AS T1 JOIN hangar AS T2 ON T1.plane_name = T2.plane_name WHERE T2.plane_name = Piper Cub ORDER BY T1.age ASC LIMIT 1
SELECT count(*) FROM pilotSkills AS T1 JOIN hangar AS T2 ON T1.plane_name = T2.plane_name WHERE T2.location = Chicago
SELECT T1.plane_name FROM Hangar AS T1 JOIN pilotskills AS T2 ON T1.plane_name = T2.plane_name GROUP BY T2.plane_name ORDER BY count(*) 1 LIMIT 1
...
SELECT T2.plane_name FROM pilot_skills AS T1 JOIN hangar AS T2 ON T1.plane_name = T2.plane_name WHERE T1.pilot_name = Jones AND T1.age = 32
...
SELECT count(DISTINCT pilot_name) FROM pilot_skills WHERE age > 40
SELECT count(DISTINCT plane_name) FROM pilotSkills
...
SELECT plane_name FROM pilot_skills WHERE pilot_name = Jones AND age = 32
...
SELECT count(*) FROM pilot_skills AS T1 JOIN hangar AS T2 ON T1.plane_name = T2.plane_name WHERE T2.location = Chicago
SELECT pilot_name FROM pilot_skills ORDER BY age DESC
...
SELECT count(*) FROM pilot_skills WHERE age > 40
...
SELECT pilot_name FROM pilot skills WHERE plane_name = Piper Cub UNION SELECT pilot_name FROM pilot skills WHERE plane_name = F-14 Fighter
...
SELECT avg(age) , plane_name FROM pilot_skills GROUP BY plane_name
SELECT max(age) , pilot_name FROM pilot_skills GROUP BY pilot_name
...
SELECT DISTINCT T1.pilot_name FROM pilot_skills AS T1 JOIN hangar AS T2 ON T1.plane_name = T2.plane_name WHERE T2.location = Piper Cub EXCEPT SELECT DISTINCT T1.pilot_name FROM pilot_skills AS T1 JOIN hangar AS T2 ON T1.plane_name = T2.plane_name WHERE T2.location = B-52
...
SELECT count(*) , avg(PilotSkills.age) , T2.location FROM PilotSkills AS T1 JOIN Hangar AS T2 ON T1.plane_name = T2.plane_name GROUP BY T2.location
SELECT pilot_name FROM pilotskills WHERE age > (SELECT min(age) FROM pilotskills WHERE plane_name = Piper Cub )
SELECT count(*) , T1.plane_name FROM pilotinfo AS T1 JOIN pilotskills AS T2 ON T1.plane_name = T2.plane_name GROUP BY T2.plane_name HAVING avg(T2.age) < 35
SELECT pilot_name FROM pilot_skills JOIN hangar ON pilot_skills.plane_name = hangar.plane_name WHERE LOCATION = Austin INTERSECT SELECT pilot_name FROM pilot_skills JOIN hangar ON pilot_skills.plane_name = hangar.plane_name WHERE LOCATION = Boston
...
SELECT count(*) , pilot_name FROM pilot_skills WHERE age < (SELECT min(age) FROM pilot_skills WHERE plane_name = 'F-14 Fighter')
...
SELECT name , speech_title FROM spokesman
...
SELECT DISTINCT count(*) , pilot_name FROM pilot_skills GROUP BY pilot_name HAVING avg(age) < 35
...
SELECT DISTINCT plane_name FROM pilot_skills WHERE pilot_name LIKE %Bomber%
...
SELECT title , YEAR FROM SCULTPRES WHERE LOCATION != gallery 226
SELECT fname , lname FROM Artists ORDER BY birthYear - deathYear LIMIT 1
SELECT max(deathyear - birthyear) FROM artists
SELECT DISTINCT T2.fname , T2.lname FROM sculptures AS T1 JOIN artists AS T2 ON T1.sculptorID = T1.artistID WHERE T1.year < 1900
...
SELECT count(*) , T3.fname FROM artists AS T1 JOIN paintings AS T2 ON T1.artistID = T2.painterID JOIN artists AS T3 ON T3.artistID = T1.artistID GROUP BY T3.fname ORDER BY deathyear - birthyear DESC LIMIT 1
...
SELECT DISTINCT T1.painterID FROM Artists AS T1 JOIN Paintings AS T2 ON T1.artistID = T2.painterID WHERE T2.medium = oil AND T2.location = 240
...
SELECT DISTINCT T2.painterID FROM paintings AS T1 JOIN artists AS T2 ON T1.painterID = T2.artistID WHERE T1.medium = oil AND T1.location = 240
SELECT DISTINCT title FROM paintings ORDER BY height_mm + width_mm
SELECT DISTINCT title FROM Paintings ORDER BY title
SELECT count(*) , T1.medium FROM artworks AS T1 JOIN paintings AS T2 ON T1.artworkid = T2.paintingid GROUP BY T1.medium UNION SELECT count(*) , T1.medium FROM artworks AS T1 JOIN sculptures AS T2 ON T1.artworkid = T2.sculptureid GROUP BY T1.medium
...
SELECT DISTINCT location FROM paintings WHERE year < 1885 OR year > 1930
SELECT T2.fname , T2.lname FROM Artists AS T1 JOIN Paintings AS T2 ON T1.artistid = T2.painterid WHERE T2.medium = 'oil' INTERSECT SELECT T2.fname , T2.lname FROM Artists AS T1 JOIN Paintings AS T2 ON T1.artistid = T2.painterid WHERE T2.medium = 'lithographic'
...
SELECT T2.deathyear FROM artists AS T1 JOIN paintings AS T2 ON T1.artistid = T2.painterid GROUP BY lname LIMIT 4
SELECT paintingid FROM paintings WHERE height_mm > (SELECT max(height_mm) FROM paintings WHERE YEAR > 1900)
SELECT paintingID FROM paintings WHERE height_mm > (SELECT max(height_mm) FROM paintings WHERE YEAR > 1900)
SELECT title , paintingID FROM paintings WHERE location = 240 ORDER BY MAX(width_mm) LIMIT 1
...
SELECT count(DISTINCTEngine) FROM driver WHERE Age > 30 OR Age < 20
SELECT Driver_Name FROM driver WHERE Constructor != Bugatti
SELECT T2.Race_Name FROM driver AS T1 JOIN race AS T2 ON T1.Driver_ID = T2.Driver_ID WHERE T1.Age >= 26
SELECT T2.Driver_Name , T2.Age FROM race AS T1 JOIN driver AS T2 ON T1.Drivers_ID = T2.Driver_ID GROUP BY T1.Drivers_ID HAVING COUNT(*) >= 2
...
SELECT name FROM languages WHERE name LIKE %ish%
SELECT Driver_name FROM driver EXCEPT SELECT T1.Driver_name FROM driver AS T1 JOIN race AS T2 ON T1.driver_id = T2.driver_id WHERE T1.Drivers_name = James Hinchcliffe
...
SELECT max(health_score) , min(health_score) FROM countries WHERE country_id != (SELECT id FROM countries WHERE name = Norway )
...
SELECT avg(T2.overall_score) FROM languages AS T1 JOIN official_languages AS T2 JOIN countries AS T3 ON T1.id = T2.language_id AND T2.country_id = T3.id WHERE T1.name = English
...
SELECT T2.name , count(B.language_id) FROM countries AS t1 JOIN official_languages AS t2 ON t1.id = t2.country_id JOIN languages AS t3 ON t3.id = t2.language_id GROUP BY t3.id
...
SELECT avg(T2.room_count) FROM property_photos AS T1 JOIN properties AS T2 ON T1.property_id = T2.property_id JOIN features AS T3 ON T3.feature_id = T1.feature_id WHERE T3.feature_name = garden
SELECT T3.town_city FROM property_features AS T1 JOIN features AS T2 ON T1.feature_id = T2.feature_id JOIN addresses AS T3 ON T3.address_id = T1.property_address_id WHERE T2.feature_name = Swimming Pool
...
SELECT avg(product) FROM (SELECT count(*) , T2.property_id FROM Rooms AS T1 JOIN Properties AS T2 ON T1.property_id = T2.property_id GROUP BY T2.property_id)
...
SELECT T1.line_1_number_building , T1.line_2_number_street , T2.town_city FROM addresses AS T1 JOIN user_property_history AS T2 ON T1.address_id = T2.user_id WHERE T1.country = UK GROUP BY T1.address_id HAVING T1.town_city = Doldrum AND T1.line_1_number_building = Mildy
...
SELECT T1.datestamp , T2.property_name FROM User_Property_History AS T1 JOIN Properties AS T2 ON T1.property_id = T2.property_id ORDER BY T1.datestamp
SELECT age_category_description FROM Ref_Age_Categories WHERE age_category_code = Over 60
SELECT sum(T2.price_max) FROM USER_SOURCES AS T1 JOIN properties AS T2 ON T1.user_id = T2.owner_user_id WHERE T1.user_category_code = single mothers or students
...
SELECT T2.country FROM USERS AS T1 JOIN Addresses AS T2 ON T1.user_address_id = T2.address_id WHERE T1.first_name = Robbie
SELECT T2.property_type_description , T1.property_type_code FROM Ref_Property_Types AS T1 JOIN Properties AS T2 ON T1.property_type_code = T2.property_type_code GROUP BY T2.property_type_code ORDER BY count(*) LIMIT 1
...
SELECT T2.property_type_description , T1.property_type_code FROM Ref_Property_Types AS T1 JOIN Properties AS T2 ON T1.property_type_code = T2.property_type_code GROUP BY T1.property_type_code ORDER BY count(*) DESC LIMIT 1
SELECT T2.last_name , T1.user_id FROM User_Property_History AS T1 JOIN USERS AS T2 ON T1.user_id = T2.user_id GROUP BY T1.user_id HAVING count(*) >= 2 INTERSECT SELECT T1.user_id FROM USER_SEARCHES AS T1 GROUP BY T1.user_id HAVING count(*) <= 2
...
SELECT T2.id , T2.name FROM bike AS T1 JOIN cyclists_own_bikes AS T2 ON T1.id = T2.bike_id WHERE T1.product_name = 'racing' GROUP BY T2.id HAVING COUNT (*) >= 4
SELECT DISTINCT T3.product_name FROM user AS T1 JOIN cyclists_own_bikes AS T2 ON T1.id = T2.cyclist_id JOIN bike AS T3 ON T2.bike_id = T3.id WHERE T1.nation = 'Russia' OR T1.nation = 'Great Britain'
...
SELECT id FROM goods WHERE price < 3
SELECT count(*) , T1.id FROM cyclist AS T1 JOIN cyclist_owns_bikes AS T2 ON T1.id = T2.cyclist_id GROUP BY T1.id ORDER BY T1.id
...
SELECT DISTINCT T2.product_name FROM cyclists AS T1 JOIN cyclists_own_bikes AS T3 ON T1.id = T3.cyclist_id JOIN bike AS T2 ON T2.id = T3.bike_id WHERE T1.result > 4:21.558 AND T1.nation = austria
SELECT T2.id , T1.name FROM (SELECT T3.cyclistid , count(*) FROM bike AS T1 JOIN cyclists_own_bikes AS T2 ON T1.id = T2.bikeid JOIN cyclist AS T3 ON T2.cyclistid = T3.id JOIN bike AS T1 ON T1.id = T2.bikeid GROUP BY T3.cyclistid ORDER BY count(*) DESC LIMIT 1) AS T4 JOIN cyclist AS T1 ON T4.cyclistid = T1.id JOIN cyclists_own_bikes AS T2 ON T1.id = T2.cyclistid
SELECT DISTINCT T2.CustomerId FROM receipts AS T1 JOIN customers AS T2 ON T1.CustomerId = T2.Id JOIN items AS T3 ON T3.Receipt = T1.ReceiptNumber JOIN goods AS T4 ON T4.id = T3.Item WHERE T4.flavor = Lemon
SELECT T2.flavor , min(T2.price) FROM goods AS T1 JOIN receipt_items AS T2 ON T1.id = T2.receipt JOIN customers AS T3 ON T3.id = T2.customer GROUP BY T2.food
SELECT Item , T1.Flavor FROM items AS T1 JOIN receipt_items AS T2 ON T1.id = T2.id WHERE T1.food = cake ORDER BY T1.price DESC LIMIT 1
SELECT DISTINCT customerId FROM receipts GROUP BY CustomerId HAVING count(*) >= 15
SELECT count(*) FROM goods WHERE FLAVOR = Cake
SELECT t2.name , t2.price FROM bike AS t2 JOIN cyclists_own_bikes AS t1 ON t1.bike_id = t2.id JOIN cyclist AS t3 ON t1.cyclist_id = t3.id WHERE t3.name = Bradley Wiggins INTERSECT SELECT t2.name , t2.price FROM bike AS t2 JOIN cyclists_own_bikes AS t1 ON t1.bike_id = t2.id JOIN cyclist AS t3 ON t1.cyclist_id = t3.id WHERE t3.name = Antonio Tauler
...
SELECT count(T1.id) , T3.flavor FROM customers AS T1 JOIN receipts AS T2 ON T1.id = T2.customerid JOIN items AS T3 ON T2.receiptnumber = T3.receipt GROUP BY T3.food
SELECT T3.id FROM customers AS T3 JOIN receipts AS T1 ON T3.id = T1.customerid JOIN items AS T2 ON T2.receipt = T1.receipt WHERE T2.item IN (SELECT id FROM items WHERE ITEM = bakery GROUP BY id HAVING count(*) >= 15)
SELECT T3.food , count(*) FROM items AS T1 JOIN goods AS T2 ON T1.Item = T2.Id JOIN customers AS T3 ON T3.id = T4.customerid JOIN receipts AS T4 ON T3.id = T4.customerid GROUP BY T3.food
...
SELECT T1.ReceiptNumber , T1.Date FROM receipts AS T1 JOIN items AS T2 ON T1.ReceiptNumber = T2.Receipt ORDER BY T2.price DESC LIMIT 1
...
SELECT T3.ReceiptNumber , T3.Date FROMitems AS T1 JOINgoods AS T2 ON T1.item = T2.id JOINreceipts AS T3 ON T3.ReceiptNumber = T1.receipt GROUP BY T1.receipt ORDER BY max(T2.price) ASC LIMIT 1
SELECT DISTINCT id , price FROM goods WHERE price < (SELECT avg(price) FROM goods)
...
SELECT id FROM goods WHERE id LIKE %APP%
SELECT DISTINCT id FROM good WHERE price < (SELECT max(price) FROM good WHERE Food = Tart )
...
SELECT DISTINCT T2.ReceiptNumber FROM receipts AS T1 JOIN items AS T2 ON T1.ReceiptNumber = T2.Receipt JOIN goods AS T3 ON T2.Item = T3.Id WHERE T3.Price > 13
...
SELECT DISTINCT T1.ReceiptNumber FROM receipts AS T1 JOIN items AS T2 ON T1.ReceiptNumber = T2.ReceiptNumber WHERE T2.Item IN (SELECT T3.id FROM goods AS T3 WHERE T3.food = cake ) INTERSECT SELECT DISTINCT T1.ReceiptNumber FROM receipts AS T1 JOIN items AS T2 ON T1.ReceiptNumber = T2.ReceiptNumber WHERE T2.Item IN (SELECT T3.id FROM goods AS T3 WHERE T3.food = cookie )
...
SELECT date FROM receipts GROUP BY date ORDER BY count(*) DESC LIMIT 3
SELECT Price FROM goods WHERE FOLLOWING = 'cheese' ORDER BY Price ASC LIMIT 1
SELECT T3.date FROM items AS T1 JOIN customers AS T2 ON T1.receipt = T2.receipt JOIN receipts AS T3 ON T1.receipt = T3.receipt JOIN goods AS T4 ON T1.item = T4.id WHERE T4.price > 15
...
SELECT T2.CustomerId , count(*) FROM receipts AS T1 JOIN customers AS T2 ON T1.CustomerId = T2.Id GROUP BY T2.CustomerId ORDER BY count(*) DESC LIMIT 1
...
SELECT T4.receiptnumber FROM items AS T1 JOIN customers AS T2 ON T1.receipt = T2.ReceiptNumber JOIN good AS T3 ON T1.Item = T3.id JOIN receipts AS T4 ON T4.receiptnumber = T1.receipt WHERE T3.price > 10 UNION SELECT receiptnumber FROM receipts ORDER BY date LIMIT 1
SELECT count(*) , T2.Date FROM receipts AS T1 JOIN customers AS T2 ON T1.CustomerId = T2.Id GROUP BY T1.Date
SELECT id FROM goods WHERE price >= avg(price) AND food = Tart
SELECT id , food , price FROM goods ORDER BY price
SELECT Item FROM items AS T1 JOIN receipts AS T2 ON T1.Receipt = T2.Receipt WHERE T1.Item IN (SELECT Item FROM receipt WHERE T2.Date = 2014-12-25 )
SELECT id FROM orders WHERE price > (SELECT avg(price) FROM orders) * 2
SELECT count(DISTINCT t2.customerid) , t2.date FROM receipts AS t1 JOIN customers AS t2 ON t1.customerid = t2.id GROUP BY t2.date
SELECT id , flavor , food FROM goods ORDER BY price
SELECT id FROM products WHERE price > (SELECT avg(price) * 2)
...
SELECT T1.flavor FROM goods AS T1 JOIN R1 JOIN T2 ON T1.id = T2.id WHERE T1.food = Cake EXCEPT SELECT T1.flavor FROM T1 JOIN T2 JOIN T3 ON T1.id = T2.id WHERE T1.food = Tart
SELECT id FROM items EXCEPT SELECT id FROM receipts
...
SELECT T1.flavor FROM goods AS T1 JOIN items AS T2 ON T1.id = T2.id GROUP BY T2.id ORDER BY count(*) DESC LIMIT 3
SELECT count(*) FROM driver
SELECT count(*) FROM driver
SELECT count(*) , make FROM driver WHERE Points > 150 GROUP BY make
SELECT avg(Age) , Make FROM driver GROUP BY Make
SELECT T2.Date FROM items AS T1 JOIN receipts AS T2 ON T1.receipt = T2.receiptnumber GROUP BY T2.receiptnumber HAVING sum(T1.ord * T1.Item) > 100
SELECT count(*) , Team FROM team_driver AS T1 JOIN driver AS T2 ON T1.Driver_ID = T2.Driver_ID WHERE T2.Points > 150 GROUP BY T2.Make
SELECT avg(Age) , Make FROM driver GROUP BY Make
SELECT avg(Laps) FROM driver WHERE Age < 20
SELECT Manager , Sponsor FROM Team ORDER BY Car_Owner
SELECT avg(Laps) FROM driver WHERE Age < 20
SELECT Manager , Sponsor FROM team ORDER BY Car_Owner
SELECT T3.id , T4.id FROM receipts AS T1 JOIN customers AS T2 ON T1.customerid = 2 JOIN items AS T3 ON T3.receipt = T1.receipt JOIN goods AS T4 ON T3.id = T4.id GROUP BY T4.id HAVING avg(T4.price) > 5
SELECT T2.customerid FROM receipts AS T1 JOIN customers AS T2 ON T1.customerid = T2.id JOIN items AS T3 ON T1.receiptnumber = T3.receipt GROUP BY T2.customerid HAVING avg(T3.price) > 5
...
SELECT DISTINCT T1.id FROM customers AS T1 JOIN receipts AS T2 ON T1.id = T2.customerid JOIN item AS T3 ON T3.Receipt = T2.ReceiptNumber JOIN goods AS T4 ON T4.id = T3.id GROUP BY T1.id HAVING sum(T4.price) > 150
SELECT DISTINCT T1.flavor FROM goods AS T1 JOIN items AS T2 ON T1.id = T2.id WHERE T2.food = Cake EXCEPT SELECT DISTINCT T1.flavor FROM goods AS T1 JOIN items AS T2 ON T1.id = T2.id WHERE T2.food = Tart
...
SELECT t3.nickname FROM institution AS t1 JOIN game AS t2 ON t1.Institution_ID = t2.Institution_ID JOIN championship AS t3 ON t1.Institution_ID = t3.Institution_ID ORDER BY t1.Enrollment ASC LIMIT 1
...
SELECT t1.nickname FROM institution AS t1 JOIN championship AS t2 ON t1.institution_id = t2.institution_id ORDER BY t1.enrollment ASC LIMIT 1
SELECT sum(t1.Number_of_Championships) FROM institution AS t2 JOIN competition AS t1 ON t2.Institution_ID = t1.Institution_ID WHERE t2.Affiliation = 'Public'
...
SELECT sum(Value) , Left(Contents , 1) FROMBoxes GROUP BY sum(Value) ORDER BY sum(Value) DESC LIMIT 1
...
SELECT DISTINCT warehouse FROM boxes WHERE contents = Rocks OR = Scissors
...
SELECT DISTINCT T1.code , T1.Contents FROM Boxes AS T1 JOIN Boxes AS T2 ON T1.code = T2.code WHERE T1.Code != T2.code AND T1.Contents = 'Scissors' AND T1.content > (SELECT max(value) FROM boxes WHERE T1.contents = 'scissors')
...
SELECT T1.Rank , T3.Major_Name FROM university AS T1 JOIN major_ranking AS T2 ON T1.University_ID = T2.University_ID JOIN major AS T3 ON T2.Major_ID = T3.Major_ID WHERE T1.University_Name = Augustana College
SELECT t2.major_name , t1.major_rank FROM major_ranking AS t1 JOIN major AS t2 ON t1.major_id = t2.major_id JOIN university AS t3 ON t3.university_id = t1.university_id WHERE t3.university_name = Augustana College
...
SELECT T1.University_Name , T1.Rank FROM overall_ranking AS T1 JOIN university AS T2 ON T1.University_ID = T2.University_ID WHERE T2.State = Wisconsin
...
SELECT COUNT ( DISTINCT T1.university_name ) FROM university AS T1 JOIN major AS T2 ON T1.university_id = T2.university_id JOIN major_ranking AS T3 ON T2.major_id = T3.major_id WHERE T2.major_name = Accounting INTERSECT SELECT COUNT ( DISTINCT T1.university_name ) FROM university AS T1 JOIN major AS T2 ON T1.university_id = T2.university_id JOIN major_ranking AS T3 ON T2.major_id = T3.major_id WHERE T2.major_name = Urban Education
...
SELECT T2.title FROM MovieTheaters AS T1 JOIN Movies AS T2 ON T1.Movie = T2.code WHERE T1.Name = Odeon
SELECT title FROM Movies WHERE code NOT IN (SELECT movie FROM MovieTheathers)
SELECT T2.title , T1.name FROM MovieTheaters AS T1 JOIN Movies AS T2 ON T1.Movie = T2.Code
...
SELECT T2.Title , T1.Rating FROM MovieTheaters AS T1 JOIN Movies AS T2 ON T1.Movie = T2.Code WHERE T2.Title LIKE '%Citizen%'
SELECT title FROM Movies ORDER BY Rating
SELECT Title FROM Movies ORDER BY Title
...
SELECT T2.title FROM MovieTheaters AS T1 JOIN Movies AS T2 JOIN MovieTheaters AS T3 ON T1.movie = T2.code AND T3.movie = T2.code WHERE T1.code = T2.code INTERSECT SELECT T2.title FROM MovieTheaters AS T1 JOIN Movies AS T2 ON T1.movie = T2.code GROUP BY T2.code ORDER BY count(*) DESC LIMIT 1
...
SELECT T1 package_number FROM Package AS T1 JOIN Client AS T2 ON T1.Sender = T2.AccountNumber JOIN Employee AS T3 ON T3.EmployeeID = T2.AccountNumber WHERE T3.Name = Leo Wong
SELECT T2.PackageNumber FROM Client AS T1 JOIN Package AS T2 ON T1.AccountNumber = T2.Receiver WHERE T1.Name = Leo Wong
...
SELECT DISTINCT package_number FROM employee AS T1 JOIN package AS T2 ON T1.EmployeeID = T2.Sender UNION SELECT DISTINCT package_number FROM employee AS T1 JOIN package AS T2 ON T1.EmployeeID = T2.Recipient WHERE T1.name = Leo Wong
...
SELECT count(*) FROM Package AS T1 JOIN Client AS T2 ON T1.Sender = T2.AccountNumber JOIN Client AS T3 ON T1.Recipient = T3.AccountNumber JOIN Shipment AS T4 ON T1.Shiptype = T4.Shiptype JOIN Employee AS T5 ON T4.Manager = T5.EmployeeID WHERE T2.Name = Ogden Wernstrom AND T3.Name = Leo Wong
...
SELECT T1.Level FROM Employee AS T1 JOIN Has_clearance AS T2 ON T1.EmployeeID = T2.EmployeeID JOIN Position AS T3 ON T3.Position = Physician
SELECT packageNumber , Weight FROM PACKAGE ORDER BY Weight ASC LIMIT 3
SELECT T3.Contents FROM Client AS T1 JOIN Package AS T2 JOIN Shipment AS T3 ON T1.AccountNumber = T2.Sender AND T3.ShipmentID = T2.Shipment JOIN Employee AS T4 ON T3.Manager = T4.EmployeeID WHERE T1.Name = John Zoidfarb
...
SELECT T2.name , sum(T1.weight) FROM Package AS T1 JOIN Planet AS T2 ON T1.Shipment = T3.Shipment JOIN Shipment AS T3 ON T2.PlanetID = T3.Planet GROUP BY T2.name
SELECT name FROM employee EXCEPT SELECT T1.name FROM employee AS T1 JOIN has_clearance AS T2 ON T1.EmployeeID = T2.Employee JOIN planet AS T3 ON T3.PlanetID = T2.Planet WHERE T3.name = Mars
...
SELECT T1.name FROM planet AS T1 JOIN shipment AS T2 ON T1.PlanetID = T2.Planet WHERE T2.Date > 2010 GROUP BY T2.Planet HAVING SUM(T3.Weight) > 30
SELECT count(*) FROM Player AS T1 JOIN Planet AS T2 JOIN Shipment AS T3 JOIN Package AS T4 JOIN Client AS T5 ON T1.Name = Zapp Brannigan AND T2.Name = Omicron Persei 8 AND T3.Planet = T2.PlanetID AND T3.Manager = T1.EmployeeID AND T4.Shipment = T3.ShipmentID AND T4.Sender = T5.AccountNumber
SELECT Name FROM Employee WHERE Salary BETWEEN 5000 AND 10000
SELECT T1.name FROM planet AS T1 JOIN has_clearance AS T2 ON T1.planetid = T2.planet GROUP BY T2.planet HAVING count(*) = 1
SELECT T1.Name FROM Employee AS T1 JOIN Has_clearance AS T2 ON T1.EmployeeID = T2.Employee JOIN planet AS T3 ON T3.PlanetID = T2.pradio WHERE T3.name = Omega III
SELECT Name FROM Employee WHERE Salary BETWEEN 5000 AND 10000
SELECT Name FROM Employee WHERE Salary > (SELECT avg(Salary) FROM Employee) OR Salary > 5000
SELECT T3.PackageNumber FROM Shipment AS T1 JOIN Employee AS T2 JOIN Package AS T3 ON T1.Manager = T2.EmployeeID AND T1.ShiptID = T3.ShiptID JOIN Planet AS T4 ON T1.Planet = T4.PlanetID WHERE T2.Name = Zapp Brannigan AND T4.Name = Omicron Persei 8
SELECT Name FROM Employee WHERE salary > (SELECT avg(salary) FROM Employee) OR salary > 5000
...
SELECT count(*) FROM Shipment AS T1 JOIN shipper AS T2 ON T1.ShipmentID = T2.ShipmentID JOIN Client AS T3 ON T3.AccountNumber = T2.Recipient WHERE T1.manager = Tucker INTERSECT SELECT count(*) FROM Shipment AS T1 JOIN shipper AS T2 ON T1.ShipmentID = T2.ShipmentID JOIN Client AS T3 ON T3.AccountNumber = T2.Recipient WHERE T1.manager = Tucker AND T1.Planet = T3.Name
SELECT Platform_name , market_district FROM platform
SELECT avg(Units_sold_Millions) FROM game WHERE Developers != Nintendo
SELECT player_name , rank_of_the_year FROM player WHERE rank_of_the_year = 1
SELECT count(*) FROM Employee WHERE EmployeeID NOT IN (SELECT T1.Employee FROM Has_Clearance AS T1 JOIN Employee AS T2 ON T1.Employee = T2.Employee WHERE T1.Planet = (SELECT PlanetID FROM Planet WHERE Name = Mars ))
...
SELECT max(ranking_of_the_year) , min(ranking_of_the_year) FROM player
...
SELECT DISTINCT T3.Developers FROM game_player AS T1 JOIN player AS T2 ON T1.Player_ID = T2.Player_ID JOIN game AS T3 ON T1.Game_ID = T3.Game_ID WHERE T2.College = Auburn
SELECT T2.Player_name , T2.Rank_of_the_year FROM game AS T1 JOIN game_player AS T2 JOIN player AS T3 ON T1.Game_ID = T2.Game_ID AND T2.Player_ID = T3.Player_ID WHERE T1.title = Super Mario World
...
SELECT T1.AccountNumber FROM Client AS T1 JOIN Package AS T2 ON T1.AccountNumber = T2.Sender WHERE T2.Recipient = (SELECT T2.AccountNumber FROM Client AS T2 JOIN Package AS T3 ON T2.AccountNumber = T3.Sender JOIN Shipment AS T4 ON T3.Shipment = T4.Shipment JOIN Has_Clearance AS T5 ON T4.Manager = T5.Employee JOIN Employee AS T6 ON T5.Employee = T6.EmployeeID WHERE T4.Date > 1512-05-21 AND T4.Manager = 3 AND T4.Planet = 1) UNION SELECT T1.AccountNumber FROM Client AS T1 JOIN Package AS T2 ON T1.AccountNumber = T2.Recipient JOIN Shipment AS T3 ON T2.ShipMENT = T3.ShipMENT JOIN Has_Clearance AS T4 ON T3.Manager = T4.Employee JOIN Employee AS T5 ON T4.Employee = T5.EmployeeID WHERE T3.Date > 1512-05-21 AND T3.Manager = 3 AND T3.Planet = 1
...
SELECT title , T1.Platform_name FROM game AS T1 JOIN platform AS T2 ON T1.Platform_ID = T2.Platform_ID
...
SELECT count(*) FROM Employee WHERE EmployeeID NOT IN (SELECT Manager FROM Has_Clearance WHERE Level = 5 AND Has_Clearance.Plant = (SELECT PlanetID FROM Planet WHERE name = Mars ))
...
SELECT Name FROM press ORDER BY Month_Profits_billion LIMIT 1 UNION SELECT Name FROM press ORDER BY Month_Profits_billion ASC LIMIT 1
SELECT T2.Name FROM book AS T1 JOIN author AS T2 ON T1.Author_ID = T2.Author_ID WHERE T1.Sale_Amount = (SELECT max(Sale_Amount) FROM book)
...
SELECT title , name , name FROM book AS T1 JOIN author AS T2 ON T1.author_id = T2.author_id JOIN press AS T3 ON T1.press_id = T3.press_id ORDER BY T1.sale_amount DESC LIMIT 3
...
SELECT T1.process_outcome_description , T2.process_status_description FROM Process_outcomes AS T1 JOIN Document_processes AS T3 JOIN Process_status AS T2 ON T3.process_outcome_code = T1.process_outcome_code AND T3.process_status_code = T2.process_status_code WHERE T3.document_id = 0
...
SELECT T1.agency_id , T1.agency_details FROM Agenciew AS T1 JOIN Clients AS T2 ON T1.agency_id = T2.agency_id GROUP BY T1.agency_id ORDER BY count(*) DESC LIMIT 1
SELECT T1.agency_details FROM agencies AS T1 JOIN clients AS T2 ON T1.agency_id = T2.agency_id WHERE T2.client_details = 'Mac'
...
SELECT T2.agency_id , T2.agency_details FROM Clients AS T1 JOIN Agencies AS T2 JOIN ON T1.agency_id = T2.agency_id GROUP BY T2.agency_id HAVING count(*) >= 2
SELECT invoice_status , count(*) FROM invoices GROUP BY invoice_status
...
SELECT payment_id , invoice_id , invoice_details FROM Payments
...
SELECT DISTINCT invoice_id , T1.invoice_status FROM invoices AS T1 JOIN payments AS T2 ON T1.invoice_id = T2.invoice_id
SELECT count(*) FROM staff
...
SELECT DISTINCT invoice_id , T2.invoices_status FROM Payments AS T1 JOIN invoices AS T2 ON T1.invoice_id = T2.invoice_id
SELECT T1.agency_id , T1.agency_details FROM Agencies AS T1 JOIN Staff AS T2 ON T1.agency_id = T2.agency_id GROUP BY T1.agency_id ORDER BY count(*) DESC LIMIT 1
...
SELECT T1.agency_id , count(*) FROM STAFF AS T1 JOIN meetings AS T2 ON T1.staff_id = T2.staff_id GROUP BY T1.agency_id
SELECT T1.agency_id , count(*) FROM Staff AS T1 JOIN Meetings AS T2 ON T1.staff_id = T2.staff_id GROUP BY T1.agency_id
...
SELECT T2.start_date_time , T2.end_date_time , T1.client_details , T3.staff_details FROM Clients AS T1 JOIN Meetings AS T2 ON T1.client_id = T2.client_id JOIN Staff AS T3 ON T3.staff_id = T2.staff_id
